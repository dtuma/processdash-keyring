...
public class KeyringTest {
    ...
    public static void main(String[] args) {
        ...
        for (String[] oneTest : TEST_PASSWORDS) {
            ...
            if (password.equals(retrievedPassword))
                msg("Successfully stored/retrieved password for " + key);
            else
                msg("############ Could not store/retrieve password for " + key);
        }
        ...
        for (String[] oneTest : TEST_PASSWORDS) {
            ...
            char[] retrieved = Keyring.read(key);
            if (retrieved == null)
                msg("############ Lost password for " + key);

            Keyring.delete(key);
            retrieved = Keyring.read(key);
            if (retrieved != null)
                msg("############ Could not delete password for " + key);
            else
                msg("Successfully deleted password for " + key);
        }
    }
    ...
    private static final String[][] TEST_PASSWORDS = {
        { "john", "my password" },
        { "someUser@http://pdes.example.com/DataBridge/INST-3nrh72",
          "This is a longer password to test and make certain we can "
                + "handle longer strings.  The quick brown fox jumped "
                + "over the lazy dogs." },
        { "zhang", "\u8FC7\u7A0B\u5EA6\u91CF\u677F" },      
    };
    ...
    private static void setupLogging() {
        ...
        try {
            InputStream config = KeyringTest.class
